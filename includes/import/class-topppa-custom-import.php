 ...
 */
class TOPPPA_Custom_Import extends WP_Importer {
	...
	public function import_start( $file ) {
		if ( ! is_file( $file ) ) {
			echo '<p><strong>' . esc_html__( 'Sorry, there has been an error.', 'topper-pack' ) . '</strong><br />';
			echo esc_html__( 'The file does not exist, please try again.', 'topper-pack' ) . '</p>';
			$this->footer();
			die();
		}
	...
	public function import_end() {
		...
		echo '<p>' . esc_html__( 'All done.', 'topper-pack' ) . ' <a href="' . esc_url( admin_url() ) . '">' . esc_html__( 'Have fun!', 'topper-pack' ) . '</a>' . '</p>';
		echo '<p>' . esc_html__( 'Remember to update the passwords and roles of imported users.', 'topper-pack' ) . '</p>';
	...
	public function handle_upload() {
		...
		if ( isset( $file['error'] ) ) {
			...
			echo '<p>' . wp_kses_post( sprintf(
				// translators: %s: File path that could not be found.
				__( 'The export file could not be found at <code>%s</code>. It is likely that this was caused by a permissions problem.', 'topper-pack' ),
				esc_html( $file['file'] )
			) ) . '</p>';
			return false;
		}
		...
		if ( $this->version > $this->max_wxr_version ) {
			...
			echo wp_kses_post( sprintf(
				// translators: %s: Version of the WXR file.
				__( 'This WXR file (version %s) may not be supported by this version of the importer. Please consider updating.', 'topper-pack' ),
	...
	public function get_authors_from_import( $import_data ) {
		if ( ! empty( $import_data['authors'] ) ) {
			...
			foreach ( $import_data['posts'] as $post ) {
				...
				if ( empty( $login ) ) {
					echo wp_kses_post( sprintf(
						// translators: %s: Author login name that failed to import.
						__( 'Failed to import author %s. Their posts will be attributed to the current user.', 'topper-pack' ),
...
<?php if ( ! empty( $this->authors ) ) : ?>
	<h3><?php esc_html_e( 'Assign Authors', 'topper-pack' ); ?></h3>
	<p><?php esc_html_e( 'To make it simpler for you to edit and save the imported content, you may want to reassign the author of the imported item to an existing user of this site, such as your primary administrator account.', 'topper-pack' ); ?></p>
<?php if ( $this->allow_create_users() ) : ?>
	<p><?php
		echo wp_kses_post( sprintf(
			// translators: %s: Default WordPress role assigned to new users.
			__( 'If a new user is created by WordPress, a new password will be randomly generated and the new user&#8217;s role will be set as %s. Manually changing the new user&#8217;s details will be necessary.', 'topper-pack' ),
...
<?php if ( $this->allow_fetch_attachments() ) : ?>
	<h3><?php esc_html_e( 'Import Attachments', 'topper-pack' ); ?></h3>
	<p>
		<input type="checkbox" value="1" name="fetch_attachments" id="import-attachments" />
		<label for="import-attachments"><?php esc_html_e( 'Download and import file attachments', 'topper-pack' ); ?></label>
	</p>
<?php endif; ?>

	<p class="submit"><input type="submit" class="button" value="<?php esc_attr_e( 'Submit', 'topper-pack' ); ?>" /></p>
</form>
		<?php
	...
	public function author_select( $n, $author ) {
		// translators: 1: Import author:
		esc_html_e( 'Import author:', 'topper-pack' );
		...
		if ( $create_users ) {
			...
			if ( '1.0' != $this->version ) {
				// translators: 1: or create new user with login name:
				esc_html_e( 'or create new user with login name:', 'topper-pack' );
				$value = '';
			} else {
				// translators: 1: as a new user:
				esc_html_e( 'as a new user:', 'topper-pack' );
		...
		if ( ! $create_users && '1.0' == $this->version ) {
			// translators: 1: assign posts to an existing user:
			esc_html_e( 'assign posts to an existing user:', 'topper-pack' );
		} else {
			// translators: 1: or assign posts to an existing user:
			esc_html_e( 'or assign posts to an existing user:', 'topper-pack' );
		}
		echo '</label>';

		echo ' ' . wp_dropdown_users(
			array(
				'name'            => "user_map[" . esc_attr( $n ) . "]",
				'id'              => 'imported_authors_' . esc_attr( $n ),
				'multi'           => true,
				// translators: 1: - Select -
				'show_option_all' => esc_html__( '- Select -', 'topper-pack' ),
	...
	public function get_author_mapping() {
		// Nonce verification is handled at the dispatch level in the import process
		...
		foreach ( $imported_authors as $i => $old_login ) {
			...
			$santized_old_login = sanitize_user( sanitize_text_field( $old_login ), true );
			$old_id             = isset( $this->authors[ $old_login ]['author_id'] ) ? intval( $this->authors[ $old_login ]['author_id'] ) : false;

			if ( ! empty( $_POST['user_map'][ $i ] ) ) {
				$user = get_userdata( intval( sanitize_text_field( wp_unslash( $_POST['user_map'][ $i ] ) ) ) );
				if ( isset( $user->ID ) ) {
					if ( $old_id ) {
				...
				if ( ! is_wp_error( $user_id ) ) {
					...
					echo wp_kses_post( sprintf(
						// translators: %s: Author display name that failed to create.
						__( 'Failed to create new user for %s. Their posts will be attributed to the current user.', 'topper-pack' ),
	...
	public function process_categories() {
		...
		foreach ( $this->categories as $cat ) {
			...
			if ( ! is_wp_error( $id ) && $id > 0 ) {
				...
				echo wp_kses_post( sprintf(
					// translators: %s: Category nicename that failed to import.
					__( 'Failed to import category %s', 'topper-pack' ),
	...
	public function process_tags() {
		...
		foreach ( $this->tags as $tag ) {
			...
			if ( ! is_wp_error( $id ) ) {
				...
				echo wp_kses_post( sprintf(
					// translators: %s: Tag name that failed to import.
					__( 'Failed to import post tag %s', 'topper-pack' ),
	...
	public function process_terms() {
		...
		foreach ( $this->terms as $term ) {
			...
			if ( ! is_wp_error( $id ) ) {
				...
				echo wp_kses_post( sprintf(
					// translators: %1$s: Term taxonomy, %2$s: Term name that failed to import.
					__( 'Failed to import %1$s %2$s', 'topper-pack' ),
	...
	public function process_posts() {
		...
		foreach ( $this->posts as $post ) {
			...
			if ( ! post_type_exists( $post['post_type'] ) ) {
				printf(
					// translators: %1$s: Post title, %2$s: Invalid post type.
					esc_html__( 'Failed to import &#8220;%1$s&#8221;: Invalid post type %2$s', 'topper-pack' ),
			...
			*/
			...
			if ( $post_exists && get_post_type( $post_exists ) == $post['post_type'] ) {
				echo wp_kses_post( sprintf(
					...
					__( '%1$s &#8220;%2$s&#8221; already exists.', 'topper-pack' ),
				...
				foreach ( $post['terms'] as $term ) {
					...
					if ( ! $term_id ) {
						...
						if ( ! is_wp_error( $t ) ) {
							...
							echo wp_kses_post( sprintf(
								// translators: %1$s: Taxonomy, %2$s: Term name that failed to import.
								__( 'Failed to import %1$s %2$s', 'topper-pack' ),
								esc_html( $taxonomy ),
								esc_html( $term['name'] )
							) );
	...
	public function process_menu_item( $item ) {
		...
		if ( ! $menu_slug ) {
			// translators: 1: Menu item skipped due to missing menu slug
			esc_html_e( 'Menu item skipped due to missing menu slug', 'topper-pack' );
			echo '<br />';
			return;
		}
		...
		if ( ! $menu_id ) {
			echo wp_kses_post( sprintf(
				// translators: %s: Menu slug that is invalid.
				__( 'Menu item skipped due to invalid menu slug: %s', 'topper-pack' ),
				esc_html( $menu_slug )
			) );
			echo '<br />';
			return;
	...
	public function process_attachment( $post, $url ) {
		if ( ! $this->fetch_attachments ) {
			return new WP_Error(
				...
				esc_html__( 'Fetching attachments is not enabled', 'topper-pack' )
			);
		}
		...
		if ( $info ) {
			...
			return new WP_Error( 'attachment_processing_error', esc_html__( 'Invalid file type', 'topper-pack' ) );
		}
	...
	public function fetch_remote_file( $url, $post ) {
		...
		if ( ! $tmp_file_name ) {
			return new WP_Error( 'import_no_file', esc_html__( 'Could not create temporary file.', 'topper-pack' ) );
		}
		...
		if ( is_wp_error( $remote_response ) ) {
			...
			return new WP_Error(
				'import_file_error',
				sprintf(
					/* translators: %1$s is the WordPress error message, %2$s is the WordPress error code. */
					esc_html__( 'Request failed due to an error: %1$s (%2$s)', 'topper-pack' ),
		...
		if ( 200 !== $remote_response_code ) {
			...
			return new WP_Error(
				'import_file_error',
				sprintf(
					/* translators: %1$s is the HTTP error message, %2$s is the HTTP error code. */
					esc_html__( 'Remote server returned the following unexpected result: %1$s (%2$s)', 'topper-pack' ),
		...
		if ( ! $headers ) {
			wp_delete_file( $tmp_file_name );
			return new WP_Error( 'import_file_error', esc_html__( 'Remote server did not respond', 'topper-pack' ) );
		}
		...
		if ( 0 === $filesize ) {
			wp_delete_file( $tmp_file_name );
			return new WP_Error( 'import_file_error', esc_html__( 'Zero size file downloaded', 'topper-pack' ) );
		}

		if ( ! isset( $headers['content-encoding'] ) && isset( $headers['content-length'] ) && $filesize !== (int) $headers['content-length'] ) {
			wp_delete_file( $tmp_file_name );
			return new WP_Error( 'import_file_error', esc_html__( 'Downloaded file has incorrect size', 'topper-pack' ) );
		}
		...
		if ( ! empty( $max_size ) && $filesize > $max_size ) {
			...
			return new WP_Error( 'import_file_error', sprintf(
				/* translators: %s is the maximum attachment size. */
				esc_html__( 'Remote file is too large, limit is %s', 'topper-pack' ),
		...
		if ( ( ! $type || ! $ext ) && ! current_user_can( 'unfiltered_upload' ) ) {
			return new WP_Error( 'import_file_error', esc_html__( 'Sorry, this file type is not permitted for security reasons.', 'topper-pack' ) );
		}
		...
		if ( ! $move_new_file ) {
			wp_delete_file( $tmp_file_name );
			return new WP_Error( 'import_file_error', esc_html__( 'The uploaded file could not be moved', 'topper-pack' ) );
		}
	...
	public function remap_featured_images() {
		...
		foreach ( $this->featured_images as $post_id => $value ) {
			if ( isset( $this->processed_posts[ $value ] ) ) {
				...
				if ( $new_id != $value ) {
					update_post_meta( $post_id, '_thumbnail_id', $new_id ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
				}
			}
		}
	}
	...
	public function header() {
		echo '<div class="wrap">';
		echo '<h2>' . esc_html__( 'Import WordPress', 'topper-pack' ) . '</h2>';
		...
		if ( isset( $updates[ $basename ] ) ) {
			...
			echo '<div class="error"><p><strong>';
			printf(
				// translators: %s: New version number of the importer.
				esc_html__( 'A new version of this importer is available. Please update to version %s to ensure compatibility with newer export files.', 'topper-pack' ),
	...
	public function greet() {
		...
		echo '<p>' . esc_html__( 'Howdy! Upload your WordPress eXtended RSS (WXR) file and we&#8217;ll import the posts, pages, comments, custom fields, categories, and tags into this site.', 'topper-pack' ) . '</p>';
		echo '<p>' . esc_html__( 'Choose a WXR (.xml) file to upload, then click Upload file and import.', 'topper-pack' ) . '</p>';
