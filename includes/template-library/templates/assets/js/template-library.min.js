!function($, $ele, $i) {
    window.liteTranslated = function (e, t) {
        return elementorCommon.translate(e, null, t, topppaEditor.i18n)
    };
    ...
    $app.Behaviors.InsertTemplate = Marionette.Behavior.extend({
        ui: {
            insertButton: ".topppaTemplateLibrary_insert-button"
        },
        events: {
    ...
    $app.Views.EmptyTemplateCollection = Marionette.ItemView.extend({
        ...
        template: "#topppa-TemplateLibrary_empty",
        ui: {
    ...
    $app.Views.Loading = Marionette.ItemView.extend({
        template: "#topppa-TemplateLibrary_loading",
        id: "topppaTemplateLibrary_loading"
    }), 
    $app.Views.Logo = Marionette.ItemView.extend({
        template: "#topppa-TemplateLibrary_header-logo",
        className: "topppaTemplateLibrary_header-logo",
    ...
    $app.Views.BackButton = Marionette.ItemView.extend({
        template: "#topppa-TemplateLibrary_header-back",
        id: "elementor-template-library-header-preview-back",
        className: "topppaTemplateLibrary_header-back",
        events: function() {
            return {
                click: "onClick"
            }
    ...
    $app.Views.Menu = Marionette.ItemView.extend({
        template: "#topppa-TemplateLibrary_header-menu",
        id: "elementor-template-library-header-menu",
        className: "topppaTemplateLibrary_header-menu",
        templateHelpers: function() {
    ...
    $app.Views.ResponsiveMenu = Marionette.ItemView.extend({
        template: "#topppa-TemplateLibrary_header-menu-responsive",
        id: "elementor-template-library-header-menu-responsive",
        className: "topppaTemplateLibrary_header-menu-responsive",
        ui: {
    ...
    $app.Views.Actions = Marionette.ItemView.extend({
        template: "#topppa-TemplateLibrary_header-actions",
        id: "elementor-template-library-header-actions",
        ui: {
            sync: "#topppaTemplateLibrary_header-sync i"
        },
        events: {
    ...
    $app.Views.InsertWrapper = Marionette.ItemView.extend({
        template: "#topppa-TemplateLibrary_header-insert",
    ...
    $app.Views.Preview = Marionette.ItemView.extend({
        template: "#topppa-TemplateLibrary_preview",
        className: "topppaTemplateLibrary_preview",
        ui: function() {
            return {
                img: "> img"
            }
        ...
        onRender: function() {
            ...
            this.$el.append(t.el), this.ui.img.on("load", function() {
                e.$el.find("#topppaTemplateLibrary_loading").remove(), e.ui.img.show()
            })
        }
    }), 
    $app.Views.TemplateCollection = Marionette.CompositeView.extend({
        template: "#topppa_TemplateLibrary_templates",
        id: "topppa-TemplateLibrary_templates",
        className: function() {
            return "topppa-TemplateLibrary_templates topppa-TemplateLibrary_templates--" + $i.library.getFilter("type")
        },
        childViewContainer: "#topppa-TemplateLibrary_templates-list",
        emptyView: function() {
            return new $app.Views.EmptyTemplateCollection
        },
        ui: {
            templatesWindow: ".topppa-TemplateLibrary_templates-window",
            textFilter: "#topppaTemplateLibrary_search",
            tagsFilter: "#topppaTemplateLibrary_filter-tags",
            filterBar: "#topppaTemplateLibrary_toolbar-filter",
            counter: "#topppaTemplateLibrary_toolbar-counter"
        },
        events: {
        ...
        onTagsFilterClick: function(t) {
            var a = $(t.currentTarget),
                n = a.data("tag");
            $i.library.setFilter("tags", n), a.addClass("active").siblings().removeClass("active"), n = n ? $i.library.getTags()[n] : "Filter", this.ui.filterBar.find(".topppaTemplateLibrary_filter-btn").html(n + ' <i class="eicon-caret-down"></i>')
        },
        ...
        setTagsFilterHover: function() {
            ...
            e.ui.filterBar.hoverIntent(function() {
                e.ui.tagsFilter.addClass("topppaTemplateLibrary_filter-show"), e.ui.filterBar.find(".topppaTemplateLibrary_filter-btn i").addClass("eicon-caret-down").removeClass("eicon-caret-right")
            }, function() {
                e.ui.tagsFilter.removeClass("topppaTemplateLibrary_filter-show");
                e.ui.tagsFilter.addClass("topppaTemplateLibrary_filter-hide"), e.ui.filterBar.find(".topppaTemplateLibrary_filter-btn i").addClass("eicon-caret-right").removeClass("eicon-caret-down")
            }, {
                sensitivity: 50,
                interval: 150,
                timeout: 100
    ...
    $app.Views.Template = Marionette.ItemView.extend({
        template: "#topppa-TemplateLibrary_template",
        className: "topppa-TemplateLibrary_template",
        ui: {
            previewButton: ".topppaTemplateLibrary_preview-button, .topppa-TemplateLibrary_template-preview"
        },
        events: {
    ...
    $app.Modal = elementorModules.common.views.modal.Layout.extend({
        getModalOptions: function() {
            return {
                id: "topppaTemplateLibrary_modal",
                hide: {
                    onOutsideClick: !1,
                    onEscKeyPress: !0,
                    onBackgroundClick: !1
        ...
        getTemplateActionButton: function(e) {
            var t = e.isPro && !topppaEditor.hasPro ? "pro-button" : "insert-button";
            return viewId = "#topppa-TemplateLibrary_" + t, template = Marionette.TemplateCache.get(viewId), Marionette.Renderer.render(template)
        },
        showLogo: function(e) {
    ...
    $app.Manager = function() {
        ...
        function addButton() {
            ...
            }), $sl.find(".topppa_templates_add_button").length || $sl.find(TOPPPA_FIND_SELECTOR).before($topppaPpenLibraryButton)
        }

        function initAddButton($el) {
            var $t = $el.find(TOPPPA_FIND_SELECTOR);
            $t.length && !$el.find(".topppa_templates_add_button").length && $t.before($topppaPpenLibraryButton), $el.on("click.onAddElement", ".elementor-editor-section-settings .elementor-editor-element-add", addButton)
        }

        function onChanges(t, i) {
            i.addClass("elementor-active").siblings().removeClass("elementor-active");
            var a = $dlTOPPPAResponsiveMap[t] || $dlTOPPPAResponsiveMap.desktop;
            $(".topppaTemplateLibrary_preview").css("width", a)
        }
        ...
        function onEditorLoaded() {
            var $el = window.elementor.$previewContents,
                ...
                $el.on("click.onAddTemplateButton", ".topppa_templates_add_button", $this.showModal.bind($this)), this.channels.tabs.on("change:device", onChanges)
        }

        var o, s, d, c, m, $this = this;

        TOPPPA_FIND_SELECTOR = ".elementor-add-new-section .elementor-add-section-drag-title", 
        $topppaPpenLibraryButton = '<div class="elementor-add-section-area-button topppa_templates_add_button"> <img src="'+topppaEditor.templateLogo+'" /> </div>', 
        $dlTOPPPAResponsiveMap = {
            desktop: "100%",
            tab: "768px",
            mobile: "360px"
        }, 
        ...
        this.init = function() {
            $this.setFilter("type", topppaEditor.default_tab, !0), $ele.on("preview:loaded", onEditorLoaded.bind(this))
        }, 
        this.getTabs = function() {
            var e = this.getFilter("type");
            return tabs = JSON.parse(topppaEditor.tab_style)
        ...
        this.requestLibraryData = function(e) {
            ...
            e.forceSync && (t.data.sync = !0), elementorCommon.ajax.addRequest("get_topppa_library_data", t)
        }, 
        this.requestTemplateData = function(e, t) {
            ...
            t && jQuery.extend(!0, i, t), elementorCommon.ajax.addRequest("get_topppa_template_data", i)
        }, 
    ...
    jQuery(document).ready(function($) {
        // Function to handle PRO templates
        function handleProTemplates() {
            // Check if user has PRO access
            var hasProAccess = topppaEditor.has_pro_access || false;
            
            // Only target actual template cards, not navigation or other elements
            $(".elementor-template-library-template, .topppa-TemplateLibrary_template, [class*='template-library-template']").each(function() {
                var $template = $(this);
                
                // Check if we already processed this template
                if ($template.hasClass('pro-processed')) {
                    return; // Skip if already processed
                }
                
                // Look for PRO text specifically in the template name/title
                var $templateName = $template.find("[class*='name'], [class*='title'], h3, h4, .template-name, .template-title, .elementor-template-library-template-name");
                var hasProInName = false;
                
                $templateName.each(function() {
                    if ($(this).text().toUpperCase().includes("PRO")) {
                        hasProInName = true;
                    }
                });
                
                // Only proceed if this template actually has PRO in its name
                if (hasProInName) {
                    // Mark as processed
                    $template.addClass('pro-processed');
                    
                    // If user has PRO access, show normal template (no purchase button)
                    if (hasProAccess) {
                        // User has PRO access, show normal template functionality
                        $template.addClass('pro-user-access');
                        // Remove any existing purchase buttons
                        $template.find(".purchase-button").remove();
                        // Show insert buttons normally
                        $template.find("button:contains('Insert')").show();
                        $template.find("[class*='insert']").show();
                    } else {
                        // User doesn't have PRO access, show purchase button
                        $template.addClass('pro-no-access');
                        
                        // Hide insert buttons
                        $template.find("button:contains('Insert')").hide();
                        $template.find("[class*='insert']").hide();
                        
                        // Remove zoom preview and replace with purchase button
                        var $zoomPreview = $template.find(".topppa-TemplateLibrary_template-preview");
                        if ($zoomPreview.length > 0) {
                            var purchaseButton = $("<button class=\"purchase-button\">🚀 Purchase Pro</button>");
                            $zoomPreview.replaceWith(purchaseButton);
                        }
                        
                        // Remove zoom icons
                        $template.find(".eicon-zoom-in-bold").remove();
                    }
                }
            });
        }
        
        // Run PRO template handling immediately
        handleProTemplates();
        
        // Run every second
        setInterval(handleProTemplates, 1000);
        ...
        $(document).on("click", ".purchase-button", function(e) {
            ...
            var upgradeUrl = topppaEditor.upgrade_url || "https://topperpack.com/pricing/";
            var trackingSource = topppaEditor.tracking_source || "template_library";
            
            // Add UTM tracking parameters
            var trackingParams = {
                'utm_source': 'plugin',
                'utm_medium': 'purchase_link',
                'utm_campaign': trackingSource,
                'utm_content': 'pro_template_purchase'
            };
            
            // Add tracking parameters to URL
            var separator = upgradeUrl.indexOf('?') !== -1 ? '&' : '?';
            var trackingString = Object.keys(trackingParams).map(function(key) {
                return key + '=' + encodeURIComponent(trackingParams[key]);
            }).join('&');
            
            var finalUrl = upgradeUrl + separator + trackingString;
            
            // Track the click if tracking function exists
            if (typeof topppaTrackClick === 'function') {
                topppaTrackClick(trackingSource, 'pro_template_purchase');
            }
...
}(jQuery, window.elementor, window.topppa || {});