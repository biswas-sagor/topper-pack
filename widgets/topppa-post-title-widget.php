...
/**
 * Elementor topppa Post Title Widget.
 *
 * Elementor widget that inserts an embbedable content into the page, from any given URL.
 *
 * @since 1.0.0
 */
class TOPPPA_Post_Title_Widget extends \Elementor\Widget_Base {
    ...
    public function get_title() {
        return TOPPPA_EPWB . esc_html__('Post Title', 'topper-pack');
    }
    ...
    public function get_keywords() {
        return ['topppa', 'widget', 'title', 'post', 'topperpack'];
    }
    ...
    public function topppa_post_title_link() {
        $this->add_control(
            'post_link',
            [
                ...
                'default' => 'none',
                'options' => [
                    'pro_custom' => esc_html__('Custom (Pro)', 'topper-pack'),
    ...
    public function topppa_post_title_text_effect() {
        $this->add_control(
            'text_effect',
            [
    ...
    protected function register_controls() {
        ...
        $this->start_controls_section(
            'post_title_options',
            [
                'label' => esc_html__('Post Title', 'topper-pack'),
                'tab' => \Elementor\Controls_Manager::TAB_CONTENT,
            ]
        );
        $this->topppa_post_title_link();
        ...
        $this->add_responsive_control(
            ...
            [
                ...
                'toggle' => true,
                'selectors' => [
                    '{{WRAPPER}} .topppa-post-title-wrapper' => 'text-align: {{VALUE}};',
                ],
            ]
        );
        
        $this->topppa_post_title_text_effect();
        ...
        $this->add_group_control(
            ...
            [
                ...
                'selector' => '{{WRAPPER}} .topppa-post-title',
            ]
        );
        $this->add_responsive_control(
            'text_color',
            [
                ...
                'type' => \Elementor\Controls_Manager::COLOR,
                'selectors' => [
                    '{{WRAPPER}} .topppa-post-title, {{WRAPPER}} .topppa-post-title a' => 'color: {{VALUE}}',
                ],
            ]
        );
        $this->add_responsive_control(
            ...
            [
                ...
                'type' => \Elementor\Controls_Manager::COLOR,
                'selectors' => [
                    '{{WRAPPER}} .topppa-post-title a:hover' => 'color: {{VALUE}}',
                ],
            ]
        );
        $this->add_group_control(
    ...
    protected function render() {
        ...
        if ('none' !== $settings['post_link']) {
            $url = ('custom' === $settings['post_link'] && !empty($settings['post_custom_link']['url']))
                ? esc_url($settings['post_custom_link']['url'])
                : get_permalink();
            // translators: %s is the URL
            $title = sprintf('<a href="%s">%s</a>', esc_url($url), $title); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
        }

        $effect_class = !empty($settings['text_effect']) ? 'effect-' . esc_attr($settings['text_effect']) : '';
        $effect_style = '';

        if ('gradient' === ($settings['text_effect'] ?? 'none')) {
        ...
        if ('glossy' === ($settings['text_effect'] ?? 'none')) {
        ...
        echo '<div class="topppa-post-title-wrapper elementor-widget-heading">';
        // translators: %1$s is the heading tag, %2$s is the effect class, %3$s is the effect style, %4$s is the title
            echo sprintf(
                '<%1$s class="topppa-post-title %2$s" style="%3$s">%4$s</%1$s>',
